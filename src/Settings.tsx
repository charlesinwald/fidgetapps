...
import { Search, Upload, Plus, Trash2, Save, X, Monitor, Folder, Code, Settings as SettingsIcon, Image, FileText, Music, Video, Archive, Terminal, Globe, Mail, Camera, Phone, Clock, Calendar, Heart, Star, Home, User, Users, MapPin, Flag, Edit3, Eye, Download, ExternalLink, ChevronLeft, ChevronRight, Package } from "lucide-react";
...
const allIconNames = Object.keys(iconMap).filter(name => 
...
const commonApps = [
  ...
  { name: "LibreOffice Writer", command: "libreoffice --writer", category: "Office" },
  { name: "LibreOffice Calc", command: "libreoffice --calc", category: "Office" },
...
const colorOptions = [
  ...
  "bg-gray-500", "bg-slate-500", "bg-zinc-500", "bg-neutral-500",
  "bg-stone-500", "bg-red-600", "bg-orange-600", "bg-yellow-600",
...
const iconColorOptions = [
  ...
  "text-red-100", "text-blue-100", "text-green-100", "text-yellow-100",
  "text-purple-100", "text-pink-100", "text-indigo-100", "text-orange-100"
];
...
export default function Settings({ onClose }: SettingsProps) {
  ...
  const categories = ["All", ...Array.from(new Set(commonApps.map(app => app.category)))];
  ...
  const renderIcon = (app: AppConfig, size = "w-8 h-8") => {
    ...
    const IconComponent = iconMap[app.icon as keyof typeof lucideIcons] as React.ElementType;
    if (IconComponent) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="w-11/12 h-5/6 bg-white/10 border border-white/30 rounded-2xl shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden">
        {/* Header */}
          ...
          <div className="w-1/2 p-6 overflow-y-auto">
            {editingApp ? (
              <div className="space-y-6">
                <h3 className="text-xl font-semibold text-white">Edit Shortcut</h3>
                ...
                <div className="flex items-center gap-4 p-4 bg-white/5 rounded-lg">
                  ...
                  <div>
                    <div className="font-medium text-white">{editingApp.name}</div>
                    <div className="text-sm text-gray-300">{editingApp.command}</div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) => setEditingApp({ ...editingApp, command: e.target.value })}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span className="text-white">
                        {editingApp.isCustomIcon ? "Custom Icon" : editingApp.icon}
                      </span>
                    </button>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-6 gap-2">
                      {colorOptions.map((color) => (
                        <button
                          key={color}
                          onClick={() => setEditingApp({ ...editingApp, color })}
                          className={`w-10 h-10 rounded-lg ${color} ${
                            editingApp.color === color ? "ring-2 ring-white" : ""
                          }`}
                        />
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {iconColorOptions.map((iconColor) => (
                        <button
                          key={iconColor}
                          onClick={() => setEditingApp({ ...editingApp, iconColor })}
        ...
        <div className="flex justify-end gap-4 p-6 border-t border-white/20">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2 bg-green-500/20 hover:bg-green-500/30 text-green-100 rounded-lg transition-colors"
          >
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="grid grid-cols-8 gap-4">
                {paginatedIcons.map((iconName) => {
                  const IconComponent = iconMap[iconName] as React.ElementType;
                  return (
                    <button
                      key={iconName}
      ...
      {showAppChooser && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-3/4 h-3/4 bg-white/10 border border-white/30 rounded-2xl shadow-2xl backdrop-blur-xl flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-white/20">
              <h3 className="text-xl font-semibold text-white">Choose Application</h3>
              <button
